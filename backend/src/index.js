...
const JWT_SECRET = process.env.JWT_SECRET || (process.env.NODE_ENV === 'production' ? (()=>{ throw new Error('JWT_SECRET required in production'); })() : 'devsecret');
...
function validate(schema){
  return (req,res,next)=>{
    ...
    if(req.method === 'GET') req.query = parsed.data; else req.body = parsed.data;
    next();
  };
}
...
// Export
app.get('/api/export/db', adminOnly, (req, res) => {
  res.download(DB_PATH, 'pos.db');
});
app.get('/api/export/csv', adminOnly, async (req, res) => {
  const tables = ['settings','customers','products','sales','sale_items','payments','deliveries'];
  const archive = archiver('zip', { zlib: { level: 9 }});
  res.setHeader('Content-Type', 'application/zip');
  res.setHeader('Content-Disposition', 'attachment; filename="epfs-pos-export.zip"');
  archive.pipe(res);
...
// Import
app.post('/api/import/db', adminOnly, upload.single('dbfile'), (req, res) => {
  const file = req.file;
  if (!file) return res.status(400).json({ error: 'No file' });
  const backupPath = path.join('/app/backups', `manual-before-import-${dayjs().format('YYYYMMDD-HHmmss')}.db`);
  fs.copyFileSync(DB_PATH, backupPath);
  fs.copyFileSync(file.path, DB_PATH);
  fs.unlinkSync(file.path);
  res.json({ ok: true });
});
app.post('/api/import/seed', adminOnly, upload.single('seedfile'), (req, res) => {
  const file = req.file;
  if (!file) return res.status(400).json({ error: 'No file' });
  const raw = fs.readFileSync(file.path, 'utf-8');
  fs.unlinkSync(file.path);
  let data = {};
  try { data = JSON.parse(raw); } catch { return res.status(400).json({ error: 'Invalid JSON' }); }

  const tx = db.transaction(() => {
    if (Array.isArray(data.products)) {
      const ins = db.prepare('INSERT INTO products(name,sku,price_cents) VALUES(?,?,?)');
      data.products.forEach(p => ins.run(String(p.name), p.sku || null, Math.round(Number(p.price||0)*100)));
    }
    if (Array.isArray(data.customers)) {
      const ins = db.prepare('INSERT INTO customers(name,phone,email,address,notes) VALUES(?,?,?,?,?)');
      data.customers.forEach(c => ins.run(String(c.name), c.phone||null, c.email||null, c.address||null, c.notes||null));
    }
  });
  tx();
  res.json({ ok: true });
